    ...
    <DeviceFrame>
      <div className="absolute inset-0 p-6 text-navy">
        ...
        <label className="mt-6 block text-sm">
          ...
          <input className="mt-1 h-10 w-full rounded-md border border-[var(--pm-border)] px-3 outline-none focus:ring-2 focus:ring-navy/20" />
        </label>
        <label className="mt-3 block text-sm">
          {findAutoText({ pathIncludes: ['login'], textIncludes: ['パスワード','password'], fallback: t('login.password') })}
          <input type="password" className="mt-1 h-10 w-full rounded-md border border-[var(--pm-border)] px-3 outline-none focus:ring-2 focus:ring-navy/20" />
        </label>
